...
const Carousel = () => {
  ...
  const carouselContent = [
    ...
    },
    {
      ...
      description: "Cutting-edge weighing technology that streamlines operations and enhances productivity.",
      buttonText: "Learn More"
    },
    {
      title: "Expert Support",
      subtitle: "From Installation to Maintenance",
      description: "Comprehensive service solutions to ensure your weighing systems perform at their best.",
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      const t1 = gsap.timeline({ repeat: -1, defaults: { ease: "power2.inOut" } });

      // auto-slide each image
      images.forEach((_, i) => {
  ...
  const next = () => {
    ...
    setIndex((prev) => {
      ...
      gsap.to(innerRef.current, { xPercent: -newIndex * 100, duration: 0.6, ease: "power3.out" });
      return newIndex;
    });
    tlRef.current.restart(); // resume timeline
  };
    ...
    <div className="w-full" id="home">
      <div className="overflow-hidden relative" ref={carouRef}>
        <div className="w-full flex" ref={innerRef}>
          {images.map((item, idx) => (
            <div key={idx} className="w-full h-[calc(100vh-100px)] flex-shrink-0 relative">
              <img
                src={item}
                alt="carousel image"
          ...
          {images.map((_, idx) => (
            <button 
              ...
              onClick={() => {
                tlRef.current.pause();
                setIndex(idx);
                gsap.to(innerRef.current, { xPercent: -idx * 100, duration: 0.6, ease: "power3.out" });
                tlRef.current.restart();
              }}
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                idx === index ? "bg-blue-600 w-10" : "bg-white/50 hover:bg-white/80"
              }`}
